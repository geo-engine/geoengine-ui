import {Component, OnInit, Input, ViewContainerRef, OnChanges, ChangeDetectionStrategy} from '@angular/core';
...
/**
 * Dialog-Component
 * Checks the file-type of the comma-separated urls given as input-argument and sets up links to open dialogs. The dialogs will show the images or play the audios or videos
 */
export class DialogComponent implements OnChanges {
   ...
   */
  private openImageDialog(imageID:number) {
   ...
   */
  private openAudioDialog(audioID:number) {
    let config = new MdDialogConfig();
    config.viewContainerRef = this.viewContainerRef;
   ...
   */
  private openVideoDialog(videoID:number) {
    let config = new MdDialogConfig();
    config.viewContainerRef = this.viewContainerRef;
