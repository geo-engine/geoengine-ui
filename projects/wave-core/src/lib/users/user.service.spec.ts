...
describe('Service: User Service', () => {
    ...
    it('detects wrong credentials and sets session accordingly', () => {
        let completed = null;
        this.service.login({user: 'test', password: 'test'}).subscribe(
            (login_response) => {
    ...
    it('detects right credentials and sets session accordingly', () => {
        let completed = null;
        this.service.login({user: 'test', password: 'test_pw'}).subscribe(
            (login_response) => {
...
export class MockBackend {
    logins = [{user: 'test', email: 'test_mail', realname: 'test_name test_after', password: 'test_pw', sources: []}];
    ...
    public testLogin() {
        ...
        for (let login of this.logins) {
            if (
                request.request.body.includes('username=' + login.user + '&') &&
                ...
                this.httpTestingController
                    .match((req) => req.body.includes('request=sourcelist&'))
                    .forEach((req) => {
                ...
                this.httpTestingController
                    .match((req) => req.body.includes('request=info&'))
                    .forEach((req) => {
                        expect(req.request.body.includes('sessiontoken=mockSessionToken')).toBeTruthy();
                        req.flush({
                            email: login.email,
                ...
                this.httpTestingController.match((req) => true).forEach((req) => req.flush(req.request.body));
                return;
            }
        }
